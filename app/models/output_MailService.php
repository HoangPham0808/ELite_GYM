...
class MailService
{
    ...
    {
        // PHPMailer trong vendor/ hoặc PHPMailer/
        if (!class_exists('PHPMailer\\PHPMailer\\PHPMailer')) {
            $base = defined('ROOT_PATH') ? ROOT_PATH : dirname(__DIR__, 2);
            foreach (['vendor/phpmailer/phpmailer/src', 'PHPMailer'] as $p) {
                if (file_exists("$base/$p/PHPMailer.php")) {
                    require_once "$base/$p/Exception.php";
                    require_once "$base/$p/PHPMailer.php";
                    require_once "$base/$p/SMTP.php";
                    break;
                }
            }
        }
        ...
        try {
            ...
            error_log('[EliteGym] MailService error: ' . $mail->ErrorInfo);
            return false;
        }
    }
