...
$r = $db->prepare("SELECT customer_id FROM Customer WHERE account_id = ? LIMIT 1");
...
$my_classes = [];
if ($cid) {
    $res = $db->query("SELECT cr.class_id FROM ClassRegistration cr WHERE cr.customer_id = $cid");
...
<header class="nav scrolled" id="nav">
  <div class="nav-inner">
    ...
    <nav class="nav-links">
      ...
      <a href="<?= url('/') ?>#schedule">Gói tập</a>
      <a href="<?= url('/schedule') ?>" class="nav-active">Lớp tập</a>
      <a href="<?= url('/') ?>#checkin">Hướng dẫn</a>
...
<div class="sch-panel active" id="panel-week">
  ...
  <div class="sch-locked-wall">
    ...
    <a href="<?= url('/') ?>#schedule" class="sch-locked-cta">
...
<script>
  ...
  window.ELITE_BASE = '<?php
    $_xscheme = (!empty($_SERVER["HTTPS"]) && $_SERVER["HTTPS"] !== "off") ? "https" : "http";
    $_xhost   = $_SERVER["HTTP_HOST"] ?? "localhost";
    $_xpath   = str_replace("\\", "/", dirname($_SERVER["SCRIPT_NAME"] ?? "/index.php"));
    $_xpath   = rtrim($_xpath, "/");
    echo $_xscheme . "://" . $_xhost . $_xpath;
    unset($_xscheme, $_xhost, $_xpath);
  ?>';
  window.AI_STREAM_URL = window.ELITE_BASE + '/api/schedule/ai';
</script>
<script src="<?= asset('js/ai_stream.js') ?>"></script>
