...
<div class="wrapper">
      ...
      <div class="donut-outer">
        <svg class="donut-svg" viewBox="0 0 160 160">
          <?php
            ...
            $circ = 2 * M_PI * $R;
            $offset = 0;
            $donut_colors = ['#b5890f','#1d4ed8','#15803d','#7c3aed','#c2410c','#dc2626'];
          ...
          <text x="80" y="75" text-anchor="middle" fill="#1a1d23"
                font-size="18" font-family="Syne,sans-serif" font-weight="800"><?= $active_mem ?></text>
          <text x="80" y="91" text-anchor="middle" fill="#9ca3af" font-size="9">hội viên</text>
        </svg>
        <div class="donut-legend">
          ...
          <div style="color:var(--tm);font-size:13px">Chưa có dữ liệu</div>
          <?php endif; ?>
        </div>
      </div>
    </div>
  ...
  <div class="section-head">
    <div class="section-title"><span class="st-dot"></span> Danh sách báo cáo</div>
  </div>
      ...
      <table id="recentTable">
        ...
        <tbody>
          ...
          $statusMap = [
            ...
            'Pending'   => ['rgba(181,137,15,.1)', '#b5890f', 'Chờ thanh toán'],
          ...
          ?>
          <tr>
            <td><span style="color:var(--tm)">#<?= $row['invoice_id'] ?></span></td>
          ...
          <?php endforeach; ?>
          <?php if (empty($recent)): ?>
          <tr><td colspan="6" style="text-align:center;color:var(--tm);padding:32px">Chưa có giao dịch nào</td></tr>
          <?php endif; ?>
        </tbody>
      </table>
    </div>
...
<!-- ══ VIEW PANEL ═════════════════════════════════════════════ -->
<div class="view-overlay" id="viewPanel">
  <div class="view-drawer">

    <!-- Header -->
    <div class="vd-head">
      <div class="vd-head-left">
        <div class="vd-icon" id="vpIcon"><i class="fas fa-chart-bar"></i></div>
        <div style="min-width:0">
          <div class="vd-title" id="vpTitle">Xem báo cáo</div>
          <div class="vd-subdesc" id="vpDesc"></div>
        </div>
      </div>
      <button class="modal-close" onclick="closeViewPanel()"><i class="fas fa-times"></i></button>
    </div>
    ...
    <div class="vd-filters">
      ...
      <button class="btn btn-gold" style="flex-shrink:0" onclick="loadViewData()">
    ...
    <!-- Body -->
    <div class="vd-body">
      <div class="vd-state vd-loading" id="vpLoading">
        <i class="fas fa-spinner fa-spin fa-2x"></i>
        <div>Đang tải dữ liệu…</div>
      </div>
      <div class="table-wrap vd-table-wrap" id="vpTableWrap" style="display:none"></div>
      <div class="vd-state vd-empty" id="vpEmpty" style="display:none">
    ...
    <!-- Footer -->
    <div class="vd-foot">
      <span class="vd-count" id="vpCount"></span>
      <div class="vd-export-row">
        <div class="vd-fmt-chips">
          <button class="vd-fmt selected" data-fmt="pdf"   onclick="vpSelectFmt(this)" title="PDF">
            <i class="fas fa-file-pdf"></i> PDF
          </button>
          <button class="vd-fmt" data-fmt="excel" onclick="vpSelectFmt(this)" title="Excel">
            <i class="fas fa-file-excel"></i> Excel
          </button>
          <button class="vd-fmt" data-fmt="csv"   onclick="vpSelectFmt(this)" title="CSV">
            <i class="fas fa-file-csv"></i> CSV
          </button>
          <button class="vd-fmt" data-fmt="json"  onclick="vpSelectFmt(this)" title="JSON">
            <i class="fas fa-code"></i> JSON
          </button>
        </div>
        <button class="btn btn-ghost" onclick="closeViewPanel()">Đóng</button>
        <button class="btn btn-gold"  onclick="doExportFromView()">
  ...
  <div class="modal">
    ...
    <div class="export-options">
      ...
      <div class="export-opt" onclick="selectFormat(this)" data-fmt="excel">
        <i class="fas fa-file-excel" style="color:#15803d"></i>
