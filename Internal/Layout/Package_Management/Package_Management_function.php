...
switch ($action) {
    ...
    case 'add_package':
        ...
        if ($check->get_result()->num_rows > 0) {
            echo json_encode(['success' => false, 'message' => 'Plan name already exists']);
            exit;
        }

        $description_val = $description ?: null;

        $stmt = $conn->prepare("
            INSERT INTO MembershipPlan (plan_name, duration_months, price, description)
            VALUES (?, ?, ?, ?)
        ");
        $stmt->bind_param('sids', $plan_name, $duration, $price, $description_val);

        if ($stmt->execute()) {
            echo json_encode(['success' => true, 'message' => 'Membership plan added successfully', 'id' => $conn->insert_id]);
        } else {
    ...
    case 'update_package':
        ...
        $stmt = $conn->prepare("
            UPDATE MembershipPlan SET plan_name=?, duration_months=?, price=?, description=?
            WHERE plan_id=?
        ");
        $stmt->bind_param('sidsi', $plan_name, $duration, $price, $description_val, $id);
    ...
    case 'delete_package':
        ...
        foreach ($checks as [$chk, $label]) {
            ...
            if ($r['c'] > 0) {
                echo json_encode([
                    'success' => false,
                    'message' => "Cannot delete: plan has related data ($label)"
                ]);
                exit;
            }
        }
    ...
    case 'get_detail':
        ...
        // Thông tin cơ bản
        $stmt = $conn->prepare("SELECT * FROM MembershipPlan WHERE plan_id = ?");
        $stmt->bind_param('i', $id);
        $stmt->execute();
