/* qrcodejs - MIT License - https://github.com/davidshimjs/qrcodejs */
(function () {
  function QR8bitByte(data) {
    ...
    for (var i = 0, l = this.data.length; i < l; i++) {
      ...
      if (code > 0x10000) {
        byteArrays[0] = 0xF0 | ((code & 0x1C0000) >>> 18);
        byteArrays[1] = 0x80 | ((code & 0x3F000) >>> 12);
        byteArrays[2] = 0x80 | ((code & 0xFC0) >>> 6);
        byteArrays[3] = 0x80 | (code & 0x3F);
      } else if (code > 0x800) {
        byteArrays[0] = 0xE0 | ((code & 0xF000) >>> 12);
        byteArrays[1] = 0x80 | ((code & 0xFC0) >>> 6);
        byteArrays[2] = 0x80 | (code & 0x3F);
      } else if (code > 0x80) {
        byteArrays[0] = 0xC0 | ((code & 0x7C0) >>> 6);
        byteArrays[1] = 0x80 | (code & 0x3F);
      } else {
        byteArrays[0] = code;
      }
      this.parsedData = this.parsedData.concat(byteArrays);
    }
    if (this.parsedData.length != this.data.length) {
      this.parsedData.unshift(0xEF);
      this.parsedData.unshift(0xBB);
      this.parsedData.unshift(0xBF);
    }
  }
  ...
  window.QRCode.prototype.makeCode = function (sText) {
    ...
    this._oQRCode.make();
    this._oContext.clearRect(0, 0, this._htOption.width, this._htOption.height);
    ...
    for (var row = 0; row < nCount; row++) {
      for (var col = 0; col < nCount; col++) {
        this._oContext.fillStyle = this._oQRCode.isDark(row, col) ? this._htOption.colorDark : this._htOption.colorLight;
        this._oContext.fillRect(Math.round(col * nWidth), Math.round(row * nHeight), Math.ceil(nWidth), Math.ceil(nHeight));
      }
    }
