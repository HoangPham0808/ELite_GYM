...
var panels = document.querySelectorAll('.sch-panel');

var hashTab = window.location.hash === '#my' ? 'my' : 'week';
...
tabs.forEach(tab => {
  tab.addEventListener('click', () => {
    ...
    if (targetTab === 'my') {
      ...
      window.location.replace((window.ELITE_BASE||'') + `/schedule?week=${weekParam}&_r=${Date.now()}#my`);
      return;
    }
...
async function doClassAction(classId, action, btn) {
  ...
  try {
    ...
    // Tự tính base URL tránh undefined
    var _schBase = (window.ELITE_BASE && window.ELITE_BASE !== 'undefined')
        ? window.ELITE_BASE
        : (window.location.origin + window.location.pathname.replace(/\/schedule.*$/, ''));
    const res  = await fetch(_schBase + '/api/schedule', { method: 'POST', body: fd, credentials: 'include' });
...
function aiShowSaveBtn(outputBox, fullText, alreadySaved = false) {
  ...
  btn.addEventListener('click', () => {
    ...
    if (ok) {
      ...
      // Toast xác nhận
      showToast('✅ Đã lưu bài tập thành công!');
    } else {
