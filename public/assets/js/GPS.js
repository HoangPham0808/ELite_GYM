...
var GPS_ADMIN_API = (window.ELITE_BASE || '') + '/api/admin/setting/gps';
...
function initMap() {
    if (!window.GPS_INIT) {
        console.error('[GPS] window.GPS_INIT chưa được inject từ PHP!');
        if (typeof showToast === 'function') showToast('err', 'Lỗi cấu hình GPS — liên hệ admin');
        return;
    }
    const lat = window.GPS_INIT.lat;
    const lng = window.GPS_INIT.lng;
    const r   = window.GPS_INIT.radius;
    ...
    // Popup
    marker.bindPopup(`<strong style="color:#d4a017">📍 ${window.GPS_INIT.name}</strong><br>
