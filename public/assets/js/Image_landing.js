...
var API = window.ELITE_BASE + '/api/admin/setting/landing';

// Các biến DOM — gán trong runWhenReady, dùng được toàn file
var fileInput, dropZone, dzPreview, uploadBtn, uploadCount, uploadForm, replaceFileInput;
...
function bindUploadEvents() {
  ...
  uploadForm.addEventListener('submit', async e => {
    ...
    try {
      ...
      if (data.errors?.length) data.errors.forEach(err => showToast(err, 'error'));
    } catch {
...
function openRename(card) {
  renameTarget = card;
  document.getElementById('renameInput').value = card.querySelector('.img-name').textContent;
  ...
  replaceFileInput.addEventListener('change', async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        if (imgEl) imgEl.src = data.file_url + '?t=' + Date.now();
...
function onDrop(e) {
  ...
  s.textContent = '⚠️ Thứ tự thay đổi — nhấn "Lưu thứ tự"';
