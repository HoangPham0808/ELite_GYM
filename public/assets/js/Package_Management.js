// ===== HELPER: chạy callback ngay khi DOM sẵn sàng (hỗ trợ SPA) =====
...
// ===== CONFIG =====
var API_URL = window.ELITE_BASE + '/api/admin/package';
var currentPage = 1;
var LIMIT = 15;
...
async function loadPackageTypes() {
    try {
        const res  = await fetch(`${API_URL}?action=get_package_types`, { credentials: 'same-origin' });
        if (!res.ok) { console.error('loadPackageTypes HTTP', res.status); return; }
...
async function loadStats() {
    try {
        const res = await fetch(`${API_URL}?action=get_stats`, { credentials: 'same-origin' });
...
async function loadPackages() {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            console.error('loadPackages HTTP error', res.status, txt);
            showToast('Lỗi tải dữ liệu', 'error');
            return;
        }
        let d;
        try { d = await res.json(); } catch (je) { const txt = await res.text().catch(()=>''); console.error('loadPackages invalid JSON', je, txt); showToast('Lỗi tải dữ liệu', 'error'); return; }
...
async function savePackage() {
    ...
    try {
        ...
        if (!res.ok) { const txt = await res.text().catch(()=>''); console.error('savePackage HTTP', res.status, txt); showToast('Lỗi kết nối server', 'error'); return; }
        const data = await res.json();
        if (data.success) {
...
async function doDelete() {
    ...
    try {
        const res  = await fetch(API_URL, { method: 'POST', body, credentials: 'same-origin' });
        if (!res.ok) { const txt = await res.text().catch(()=>''); console.error('doDelete HTTP', res.status, txt); showToast('Lỗi kết nối', 'error'); return; }
        const data = await res.json();
        if (data.success) {
            showToast(data.message, 'success');
            closeConfirmModal();
...
async function openDetail(id) {
    ...
    try {
        const res = await fetch(`${API_URL}?action=get_detail&id=${id}`, { credentials: 'same-origin' });
        if (!res.ok) { document.getElementById('detailContent').innerHTML = '<p style="color:#f87171;padding:20px">Lỗi tải dữ liệu</p>'; return; }
        const d   = await res.json();
        if (!d.success) {
