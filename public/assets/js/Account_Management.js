// Tính base URL tuyệt đối từ location (tránh relative URL khi ELITE_BASE rỗng)
var _accBase = (window.ELITE_BASE && window.ELITE_BASE !== 'undefined')
    ? window.ELITE_BASE
    : (window.location.origin + window.location.pathname.replace(/\/admin.*$/, '') + '/public').replace('/public/public', '/public');
var API = _accBase + '/api/admin/account';
var LIMIT = 15;
var nvPage = 1, khPage = 1, logPage = 1;
var statsCache = {};

// ── KHỞI TẠO CHẠY TRỰC TIẾP (TƯƠNG THÍCH SPA - KHÔNG DÙNG DOMContentLoaded) ──
function initAccountManagement() {
    // Tabs điều hướng cấu trúc giao diện tài khoản
    ...
    // Đóng modal khi bấm ra ngoài vùng tối overlay
    document.querySelectorAll('.modal-overlay').forEach(o =>
        o.addEventListener('click', e => { if (e.target === o) o.classList.remove('active'); }));

    // Cập nhật trạng thái text hiển thị của nút Toggle trạng thái
    const fStatus = document.getElementById('fAccStatus');
    if (fStatus) {
    ...
    // Gán các sự kiện bộ lọc & Tìm kiếm (Filters & Search) có kiểm tra tồn tại phần tử
    if (document.getElementById('nvSearch')) {
    ...
    if (document.getElementById('khSearch')) {
    ...
    // Thực thi nạp dữ liệu lập tức ngay khi module script được chèn vào DOM
    loadStats();
    loadAccounts('staff');
...
var fmtDate = s => (!s||s==='0000-00-00') ? '—' : s.split('-').reverse().join('/');
...
async function loadAccounts(type, page) {
    ...
    try {
        ...
        renderPag(d.total, pg, d.totalPages, piId, pcId, type); 
        ...
        document.getElementById(tbId).innerHTML = `<tr><td colspan="${cols}"><div class="empty-state" style="color:var(--danger)"><i class="fas fa-exclamation-circle"></i> Không thể kết nối dữ liệu</div></td></tr>`;
    }
}
...
async function saveAccount() {
    ...
    if(!id && !user) { toast('Vui lòng nhập tên đăng nhập','warning'); return; }
    if(!id && pass.length < 6) { toast('Mật khẩu tối thiểu 6 ký tự','warning'); return; }
...
async function toggleStatus(id, username) {
    const btn = document.querySelector(`button[onclick*="toggleStatus(${id},"] i`);
    const isLocking = btn ? btn.classList.contains('fa-lock') : true;
    confirmDel(`${isLocking ? 'Khóa' : 'Mở khóa'} tài khoản <strong>${esc(username)}</strong>?`, async () => {
...
async function doResetPass() {
    ...
    if(pass.length < 6) { toast('Mật khẩu mới tối thiểu 6 ký tự','warning'); return; }
...
async function loadLoginHistory(page = logPage) {
    ...
    try {
        ...
        renderPag(d.total, page, d.totalPages, 'logPagInfo', 'logPagCtrl', 'history'); 
...
// ── PAGINATION (SỬA LỖI BIẾN TYPE TRONG SPA ENVIRONMENT) ───────────
function renderPag(total, page, totalPages, piId, pcId, targetType) {
    ...
    let clickCall = '';
    if (targetType === 'staff' || targetType === 'customer') {
        clickCall = `loadAccounts('${targetType}', {page})`;
    } else {
        clickCall = `loadLoginHistory({page})`;
    }

    let h = `<button class="page-btn" onclick="${clickCall.replace('{page}', page-1)}" ${page===1?'disabled':''}><i class="fas fa-chevron-left"></i></button>`;
    for (let i = 1; i <= totalPages; i++) {
        if (totalPages > 7 && Math.abs(i-page) > 2 && i !== 1 && i !== totalPages) {
            if (i === 2 || i === totalPages-1) h += `<button class="page-btn" disabled>…</button>`;
            continue;
        }
        h += `<button class="page-btn ${i===page?'active':''}" onclick="${clickCall.replace('{page}', i)}">${i}</button>`;
    }
    h += `<button class="page-btn" onclick="${clickCall.replace('{page}', page+1)}" ${page===totalPages?'disabled':''}><i class="fas fa-chevron-right"></i></button>`;
    ctrl.innerHTML = h;
}
...
// ── UTILS ─────────────────────────────────────────────────────────
var openModal  = id => document.getElementById(id).classList.add('active');
var closeModal = id => document.getElementById(id).classList.remove('active');
...
async function apiFetch(actionOrParams) {
    ...
    const r = await fetch(url, { credentials: 'include' }); return r.json();
}
async function apiPost(body) { const r = await fetch(API, { method:'POST', body, credentials: 'include' }); return r.json(); }
var fd = obj => { const f = new FormData(); Object.entries(obj).forEach(([k,v]) => f.append(k, v ?? '')); return f; };
