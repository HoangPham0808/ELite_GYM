...
var API = facilityApiUrl();
var LIMIT = 15;
var devPage = 1, btPage = 1;
var overdueFilter = false;
var categories = [];

// ── INIT ──────────────────────────────────────────────────────────
function initFacilitiesModule() {
    if (!document.getElementById('devSearch')) return;
    ...
    const fBtNgay = document.getElementById('fBtNgay');
    if (fBtNgay) fBtNgay.value = _facilityToday();
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initFacilitiesModule);
} else {
    initFacilitiesModule();
}
...
function validateCompletedStatus(selectId) {
    ...
    if (window.IS_HLV) { statusEl.value = 'Scheduled'; return false; }
...
function getStatusOptions() {
    ...
    if (window.IS_HLV)    return [{ v: 'Scheduled', l: '📅 Lên lịch' }];
    ...
    tb.innerHTML = rows.map(r => {
        ...
        return `<tr>
            ...
            <td><div class="action-btns">
                ...
                ${!window.IS_HLV ? `<button class="btn-icon" onclick="openDevEdit(${r.equipment_id})" title="Sửa"><i class="fas fa-pen"></i></button>` : ''}
                <button class="btn-icon" onclick="quickBaoTri(${r.equipment_id})" title="Thêm bảo trì nhanh"><i class="fas fa-wrench"></i></button>
                ${window.IS_ADMIN ? `<button class="btn-icon delete" onclick="confirmDel('Xóa thiết bị <strong>${esc(r.equipment_name)}</strong>?', () => deleteDevice(${r.equipment_id}))" title="Xóa"><i class="fas fa-trash"></i></button>` : ''}
            </div></td>
        </tr>`;
    }).join('');
}
...
function renderBtTable(rows) {
    ...
    tb.innerHTML = rows.map(r => `<tr>
        ...
        <td><div class="action-btns">
            ...
            ${window.IS_ADMIN ? `<button class="btn-icon delete" onclick="confirmDel('Xóa bản ghi bảo trì này?', () => deleteMaintenance(${r.maintenance_id}))"><i class="fas fa-trash"></i></button>` : ''}
        </div></td>
    </tr>`).join('');
}

async function openMaintenanceModal(preDevId = null) {
    document.getElementById('fBtNgay').value     = _facilityToday();
...
async function openEditMaintenance(id) {
    ...
    document.getElementById('fEditBtNgay').value     = r.maintenance_date || _facilityToday();
...
async function apiFetch(actionOrParams) {
    const base = facilityApiUrl();
    let url;
    if (actionOrParams.startsWith('action=') || actionOrParams.includes('&action=')) {
        url = `${base}?${actionOrParams}`;
    } else {
        url = `${base}?action=${actionOrParams}`;
    }
...
async function apiPost(body) {
    const r = await fetch(facilityApiUrl(), {
        method: 'POST',
        body,
        credentials: 'same-origin',
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
...
// ── IMPORT EXCEL ──────────────────────────────────────────────────
var importData = []; // parsed rows ready to submit
