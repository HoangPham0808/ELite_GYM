...
(function () {
  ...
  function apiHandler() {
    return (window.ELITE_BASE || '') + '/api/admin/review';
  }

  /* ── DOM refs (gán lại sau khi admin SPA inject HTML) ── */
  let tbody, pager, searchInp, ratingSel, repliedSel, refreshBtn;
  let sTotal, sAvg, sReplied, sPending, sDist;
  let modalBd, modalClose, modalCancel, modalSave, modalReview, replyTa, replyChars, modalMsg;
  let confirmBd, confirmMsg, confirmCancel, confirmOk, toast;
  ...
  async function apiFetch(url, opts = {}) {
    ...
    try {
      ...
      throw new Error('Server trả về dữ liệu không hợp lệ: ' + text.slice(0, 300));
    }
  }
  ...
  function starsHtml(n) {
    let h = '';
    for (let i = 1; i <= 5; i++) {
      h += `<i class="fa${i <= n ? 's' : 'r'} fa-star" style="color:${i <= n ? '#d97706' : '#e5e7eb'}"></i>`;
    }
    return h;
  }
  ...
  async function loadReviews(page = 1) {
    ...
    try {
      const json = await apiFetch(`${apiHandler()}?${params}`);
      ...
      tbody.innerHTML = emptyRow(e.message || 'Lỗi kết nối server.');
    }
  }
  ...
  function openReplyModal(rid, data) {
    ...
    modalReview.innerHTML = `
      <div style="display:flex;gap:8px;align-items:center;margin-bottom:8px">
        <div style="color:#d97706;font-size:12px">${starsHtml(+data.rating)}</div>
        <strong style="font-size:13px;color:var(--text)">${esc(data.name)}</strong>
      </div>
      <div style="font-size:13px;color:#4b5563;line-height:1.6">"${esc(data.content)}"</div>`;
  ...
  /* ── Confirm modal ────────────────────────────── */
  // type: 'del-reply' | 'del-review'
  function openConfirm(msg, cb, type) {
    confirmMsg.innerHTML = msg;
    confirmCb = cb;

    const header  = document.getElementById('rmConfirmHeader');
    const titleEl = document.getElementById('rmConfirmTitle');
    const iconEl  = document.getElementById('rmConfirmIcon');

    if (type === 'del-reply') {
      // Amber — ít nghiêm trọng hơn
      header.className  = 'rm-modal-header rm-modal-header-amber';
      titleEl.textContent = 'Xóa phản hồi';
      iconEl.className  = 'fas fa-eraser';
    } else {
      // Đỏ — xóa hoàn toàn, không thể hoàn tác
      header.className  = 'rm-modal-header rm-modal-header-danger';
      titleEl.textContent = 'Xác nhận xóa đánh giá';
  ...
  async function doDeleteReply(rid) {
    ...
    try {
      const json = await apiFetch(apiHandler() + '?ajax=1', { method: 'POST', body: fd });
  ...
  function bindDom() {
    tbody       = document.getElementById('rmTbody');
    pager       = document.getElementById('rmPager');
    searchInp   = document.getElementById('rmSearch');
    ratingSel   = document.getElementById('rmRating');
    repliedSel  = document.getElementById('rmReplied');
    refreshBtn  = document.getElementById('rmRefresh');
    sTotal      = document.getElementById('sTotal');
    sAvg        = document.getElementById('sAvg');
    sReplied    = document.getElementById('sReplied');
    sPending    = document.getElementById('sPending');
    sDist       = document.getElementById('sDist');
    modalBd     = document.getElementById('rmModalBd');
    modalClose  = document.getElementById('rmModalClose');
    modalCancel = document.getElementById('rmModalCancel');
    modalSave   = document.getElementById('rmModalSave');
    modalReview = document.getElementById('rmModalReview');
    replyTa     = document.getElementById('rmReplyTa');
    replyChars  = document.getElementById('rmReplyChars');
    modalMsg    = document.getElementById('rmModalMsg');
    confirmBd     = document.getElementById('rmConfirmBd');
    confirmMsg    = document.getElementById('rmConfirmMsg');
    confirmCancel = document.getElementById('rmConfirmCancel');
    confirmOk     = document.getElementById('rmConfirmOk');
    toast       = document.getElementById('rmToast');
    return !!tbody;
  }
  ...
  /* ── Boot (sau khi module inject vào admin SPA) ── */
  runWhenReady(initModule);
