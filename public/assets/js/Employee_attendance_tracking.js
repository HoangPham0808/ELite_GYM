// ===== CONFIG =====
var API_URL = window.ELITE_BASE + '/api/admin/employee-attendance';
var currentPage = 1;
var LIMIT = 20;
...
async function loadAttendance() {
    ...
    try {
        ...
        const d   = await res.json();
        if (!d.success) { showToast('Lỗi tải dữ liệu', 'error'); setTableLoading(false); return; }
        renderTable(d.data);
        renderPagination(d.total, d.totalPages);
        document.getElementById('tableTotal').textContent = `${d.total} nhân viên`;
    } catch(e) {
...
function setTableLoading(state) {
    if (state) {
        document.getElementById('attendanceTbody').innerHTML =
            `<tr><td colspan="6" style="text-align:center;padding:40px;color:var(--tm)">
...
function renderTable(rows) {
    ...
    tbody.innerHTML = rows.map(r => {
        ...
        // Action buttons
        const editBtn = `<button class="btn-action-sm" title="${hasChamCong ? 'Sửa chấm công' : 'Chấm công'}"
        ...
        const delBtn = hasChamCong
            ? `<button class="btn-action-sm delete" title="Xóa chấm công"
        ...
        return `<tr>
            ...
            <td>${gioVao}</td>
            <td>${gioRa}</td>
            <td style="font-size:12px;color:var(--tm);max-width:160px">
...
function renderPagination(total, totalPages) {
    ...
    info.textContent = total > 0 ? `Hiển thị ${from}-${to} trong ${total}` : 'Không có dữ liệu';
...
function openAttModal(empId, empName, gioiTinh, chamCongId, trangThai, gioVao, gioRa) {
    ...
    document.getElementById('attModalTitle').innerHTML =
        `<i class="fas fa-fingerprint" style="color:#d4a017;margin-right:8px"></i>${isEdit ? 'Sửa chấm công' : 'Chấm công'} - ${escHtml(empName)}`;
...
async function openBulkModal() {
    ...
    document.getElementById('bulkEmpItems').innerHTML =
        `<div style="padding:20px;text-align:center;color:var(--tm)"><i class="fas fa-spinner fa-spin"></i></div>`;
    ...
    try {
        ...
        if (!d.success || !d.data.length) {
            document.getElementById('bulkEmpItems').innerHTML =
                `<div style="padding:20px;text-align:center;color:var(--tm);font-size:13px">
                    <i class="fas fa-check-circle" style="color:#15803d;margin-right:6px"></i>
                    Tất cả nhân viên đã được chấm công hôm nay
                </div>`;
            return;
        }
        document.getElementById('bulkEmpItems').innerHTML = d.data.map(emp => `
            <div class="bulk-emp-item" onclick="toggleCheck(${emp.employee_id})">
                ...
                <div class="emp-avatar ${emp.gender === 'Female' ? 'female' : ''}" style="width:32px;height:32px;font-size:13px;flex-shrink:0">
                    ${getInitials(emp.full_name)}
                </div>
                <div class="bulk-emp-text">
                    <div class="bulk-emp-name">${escHtml(emp.full_name)}</div>
                    <div class="bulk-emp-id">#${emp.employee_id}</div>
                </div>
            </div>`).join('');
    } catch(e) {
        document.getElementById('bulkEmpItems').innerHTML =
            `<div style="padding:20px;text-align:center;color:var(--red);font-size:13px">Lỗi tải dữ liệu</div>`;
    }
}
...
async function saveBulk() {
    ...
    if (!checked.length) { showToast('Vui lòng chọn ít nhất một nhân viên', 'warning'); return; }
