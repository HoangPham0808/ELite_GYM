...
function fetchWorkoutStream(params, outputEl, onMeta, onDone) {
    ...
    // Tự tính base từ location — tránh phụ thuộc window.ELITE_BASE
    var _aiBase = (window.ELITE_BASE && window.ELITE_BASE !== 'undefined')
        ? window.ELITE_BASE
        : (window.location.origin + window.location.pathname.replace(/\/[^\/]*$/, '').replace(/\/schedule$/, ''));

    fetch(_aiBase + '/api/schedule/ai', {
        method:      'POST',
        credentials: 'include',
