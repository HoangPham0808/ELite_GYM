// ===== CONFIG =====
var API_URL = window.ELITE_BASE + '/api/admin/customer';
var currentPage = 1;
var LIMIT = 15;
...
// Dùng pattern kiểm tra readyState — nếu DOM đã sẵn sàng thì chạy ngay.
...
// ===== QR CODE MODAL =====
function showQRCode(customerId, fullName, phone) {
    // Tạo modal QR nếu chưa có
    if (!document.getElementById('qrModal')) {
        const modalHtml = `
        <div class="modal-overlay" id="qrModal">
            <div class="modal" style="max-width:420px;text-align:center">
                <div class="modal-header">
                    <h3><i class="fas fa-qrcode" style="color:#d4a017;margin-right:8px"></i>Mã QR Check-in</h3>
                    <button class="btn-close" onclick="closeQRModal()"><i class="fas fa-times"></i></button>
                </div>
                <div class="modal-body" id="qrModalBody" style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:24px">
                    <div id="qrNameBadge" style="font-size:16px;font-weight:700;color:#fff"></div>
                    <div id="qrSubBadge" style="font-size:13px;color:rgba(255,255,255,0.5)"></div>
                    <canvas id="qrCanvas" style="border-radius:12px;background:#fff;padding:12px;width:220px;height:220px"></canvas>
                    <div style="font-size:12px;color:rgba(255,255,255,0.35)">Khách hàng quét mã này để check-in</div>
                    <button class="btn-primary" onclick="downloadQR()" style="width:100%">
    ...
    document.getElementById('qrNameBadge').textContent = fullName;
    document.getElementById('qrSubBadge').textContent  = phone ? '📞 ' + phone : 'ID: ' + customerId;
    ...
    if (typeof QRious !== 'undefined') {
        new QRious({
            element: canvas,
            value:   'ELITEGYM_CID_' + customerId,
            size:    220,
            background: '#ffffff',
            foreground: '#1a1a2e',
            padding:    10,
        ...
        canvas.parentElement.innerHTML = `<div style="color:#f87171;padding:20px">QRious chưa được tải. Kiểm tra kết nối mạng.</div>`;
    }
...
function downloadQR() {
    const canvas = document.getElementById('qrCanvas');
    const name   = document.getElementById('qrNameBadge').textContent || 'qr';
