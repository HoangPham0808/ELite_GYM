...
var QR_API    = (window.ELITE_BASE || '') + '/api/admin/qr';
var QR_BRIDGE = (window.ELITE_BASE || '') + '/api/admin/qr/bridge';
...
var qrResultData   = null;
var todayLog       = [];
var qrCodeModal_id = null;
var currentFacing  = 'environment'; // 'environment' = camera sau, 'user' = camera trước
var availableCams  = [];
...
var FETCH_HEADERS = { 'ngrok-skip-browser-warning': 'true' };
var FETCH_OPTS   = { credentials: 'include', headers: FETCH_HEADERS };

async function fetchCustomerForCheckin(customerId) {
    showQRLoading(true, 'Đang tải thông tin...');
    try {
        const res = await fetch(`${QR_API}?action=get_checkin_info&id=${customerId}`, FETCH_OPTS);
        ...
        console.error('[QR fetchCustomer]', e);
...
async function sendCheckin(type) {
    ...
    try {
        ...
        fd.append('type', type);
        const res = await fetch(QR_API, { method: 'POST', body: fd, credentials: 'include', headers: FETCH_HEADERS });
        const d   = await res.json();
        showQRLoading(false);
        if (d.success) {
                ...
                if (targetClass) {
                    ...
                    autoCheckoutTimers[cid] = setTimeout(async () => {
                        ...
                        try {
                            ...
                            const r2 = await fetch(QR_API, { method: 'POST', body: fd2, credentials: 'include', headers: FETCH_HEADERS });
...
async function loadPhoneLink() {
    ...
    const base     = window.ELITE_BASE || '';
    const scannerPath = base.replace(/^https?:\/\/[^\/]+/, '') + '/qr-scanner';

    try {
        const r = await fetch(base + '/api/admin/qr/ngrok', {
            headers: { 'ngrok-skip-browser-warning': 'true' }
        }).then(x => x.json());

        let phoneUrl;
        if (r.success && r.ngrok_url) {
            phoneUrl = r.ngrok_url + scannerPath;
        } else if (location.hostname.includes('ngrok')) {
            phoneUrl = location.origin + scannerPath;
        } else {
            phoneUrl = null;
        }
        ...
        if (phoneUrl) {
            ...
            linkEl.innerHTML = '<span style="color:#f87171;">Cần chạy ngrok! Lệnh: <code>ngrok http --host-header=localhost 80</code></span>';
        }
    } catch(e) {
        if (location.hostname.includes('ngrok')) {
            const phoneUrl = location.origin + scannerPath;
            linkEl.textContent  = phoneUrl;
            linkEl.dataset.link = phoneUrl;
        } else {
            linkEl.innerHTML = '<span style="color:#f87171;">Không lấy được link ngrok.</span>';
        }
    }
}
...
var _origCloseQRModal = closeQRModal;
