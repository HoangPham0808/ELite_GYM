...
var API = (window.ELITE_BASE || '') + '/api/admin/setting/system';
...
// ══ UTILS ════════════════════════════════
var escHtml = s => String(s??'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
...
async function addPackageType() {
    ...
    try { const d=await(await fetch(API,{method:'POST',body:fd,credentials:'include'})).json(); if(d.ok){showToast('ok',d.msg);document.getElementById('inpName').value='';document.getElementById('inpDesc').value='';syncColor('#d4a017');document.getElementById('colorPicker').value='#d4a017';loadList();}else showToast('err',d.msg||'Thêm thất bại!'); } catch(e){showToast('err','Lỗi kết nối!');}
}
...
async function saveEdit(id) {
    ...
    try { const d=await(await fetch(API,{method:'POST',body:fd,credentials:'include'})).json(); if(d.ok){showToast('ok',d.msg);editingId=null;loadList();}else showToast('err',d.msg||'Thất bại!'); } catch(e){showToast('err','Lỗi!');}
}
async function toggleType(id){const fd=new FormData();fd.append('action','toggle');fd.append('type_id',id);try{const d=await(await fetch(API,{method:'POST',body:fd,credentials:'include'})).json();if(d.ok){showToast('ok',d.msg);loadList();}else showToast('err',d.msg);}catch(e){showToast('err','Lỗi!');}}
async function deleteType(id,name){if(!confirm(`Xác nhận xóa loại gói "${name}"?`))return;const fd=new FormData();fd.append('action','delete');fd.append('type_id',id);try{const d=await(await fetch(API,{method:'POST',body:fd,credentials:'include'})).json();if(d.ok){showToast('ok',d.msg);loadList();}else showToast('err',d.msg);}catch(e){showToast('err','Lỗi!');}}
...
function renderEqEditRow(r) {
    return `<tr id="eq-row-${r.type_id}" style="background:var(--blue-dim)">
        <td class="td-id">${r.type_id}</td>
        ...
        <td><div style="display:flex;align-items:center;gap:6px"><input class="edit-input eq-edit-input" id="eq-edit-int-${r.type_id}" type="number" value="${r.maintenance_interval}" min="1" style="width:72px"/> <span style="font-size:.78rem;color:var(--tm)">ngày</span></div></td>
        <td><div class="td-actions">
...
async function addEquipmentType() {
    ...
    try{const d=await(await fetch(API,{method:'POST',body:fd,credentials:'include'})).json();if(d.ok){showToast('ok',d.msg);document.getElementById('eqInpName').value='';document.getElementById('eqInpDesc').value='';document.getElementById('eqInpInterval').value='180';loadEqList();}else showToast('err',d.msg||'Thêm thất bại!');}catch(e){showToast('err','Lỗi kết nối!');}
}
